...
let nextGame=document.getElementById('contain_link')
nextGame.classList.add('display-non')
...
function checker(imgblock1,imgblock2){
    ...
    if(imgblock1.getAttribute('data-techno')===imgblock2.getAttribute('data-techno')){
       ...
       if(flipingcard.length==24){
        ...
        setTimeout (()=>{nextGame.classList.remove('display-non')},2000)
        setTimeout (()=>{nextGame.classList.add('diplay')},2000)
      ...
      let time= setTimeout ( ()=> {tried.textContent=n+1 + ' from 20';
        n++;},500)
...
alert('the game will finish after 10 minutes')
if(myName== null || myName==""){
myName1.textContent="Unknown"
// to reload page after specific time
 setTimeout(()=>{location.reload()},602000)
